...
import { useMedicationForm, type MedicationFormData } from '@/hooks/useMedicationForm';
  ...
  const onSubmit = async (formData: MedicationFormData) => {
    try {
      ...
      const medications: Medication[] = formData.medications.map((med) => ({
        ...med,
        id: med.id || '', // Store'da otomatik oluşturulacak
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      <div className="space-y-6">
        {fields.map((field, index) => (
          <div key={field.id} className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-medium text-gray-900">İlaç #{index + 1}</h3>
              {fields.length > 1 && (
                <button
                  type="button"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* İlaç Adı */}
              <div>
                <label className="block text-sm font-medium text-gray-700">İlaç Adı</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700">Doz</label>
                  <input
                    type="number"
              ...
              {/* Kullanım Sıklığı */}
              <div>
                <label className="block text-sm font-medium text-gray-700">Kullanım Sıklığı</label>
                <select
              ...
              <div className="md:col-span-2">
                <div className="flex items-center space-x-4">
                  ...
                  />
                  <label className="text-sm font-medium text-gray-700">Hatırlatıcı Ekle</label>
                </div>
